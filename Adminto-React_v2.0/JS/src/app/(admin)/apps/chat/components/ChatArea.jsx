...
import { Card, CardHeader, Col, Dropdown, DropdownItem, DropdownMenu, DropdownToggle } from 'react-bootstrap';
...
const MessageDropdown = ({
  ...
  return <Dropdown drop={message.from.id === toUser.id ? 'end' : 'start'} className="chat-actions">
...
const ChatArea = ({
  ...
  return <Card className="chat-content rounded-0 shadow-none mb-0">
      <CardHeader className="py-2 px-3 border-bottom">
        <div className="d-flex align-items-center justify-content-between py-1">
          <div className="d-flex align-items-center gap-2">
            {chatList.open ? <button onClick={chatList.toggle} className="btn btn-sm btn-icon btn-soft-primary d-none d-xl-flex me-2" data-bs-toggle="collapse" data-bs-target="#chat-user-list" aria-expanded="true">
                  <IconifyIcon icon='tabler:chevrons-left' className="fs-20" />
                </button> : <button onClick={chatList.toggle} className="btn btn-sm btn-icon btn-soft-primary d-none d-xl-flex me-2" data-bs-toggle="collapse" data-bs-target="#chat-user-list" aria-expanded="true">
                  <IconifyIcon icon='tabler:chevrons-right' className="fs-20" />
                </button>}
            <button onClick={chatSetting.toggle} className="btn btn-sm btn-icon btn-ghost-light text-dark d-xl-none d-flex" type="button" data-bs-toggle="offcanvas" data-bs-target="#chatUserList" aria-controls="chatUserList">
            ...
            {!selectedUser?.image && (selectedUser?.icon ? <div className={`avatar-lg rounded-circle bg-${selectedUser?.iconColor} text-white d-flex align-items-center justify-content-center`}>
                    <IconifyIcon icon={selectedUser?.icon} className="fs-20" />
                  </div> : <div className="avatar-lg rounded-circle bg-info text-white d-flex align-items-center justify-content-center">
                    <span className="fw-semibold">{getFirstTwoLetters(selectedUser?.name)}</span>
                  </div>)}
            <div>
              <h5 className="my-0 lh-base">
                <Link to="" className="text-reset">James Zavel</Link>
              </h5>
              <p className="mb-0 text-muted icons-center">
                <IconifyIcon icon='tabler:circle-filled' className={`text-${selectedUser?.activeOffline == 'Offline' ? 'danger' : 'success'} me-1`} /> {selectedUser?.activeOffline}
              </p>
            </div>
          </div>
          <div className="d-flex align-items-center gap-2">
            <Link to="" className="btn btn-sm btn-icon btn-ghost-light d-none d-xl-flex" data-bs-toggle="modal" data-bs-target="#userCall" data-bs-placement="top" title="Voice Call">
              <IconifyIcon icon='tabler:phone-call' className="fs-20" />
            </Link>
            <Link to="" className="btn btn-sm btn-icon btn-ghost-light d-none d-xl-flex" data-bs-toggle="modal" data-bs-target="#userVideoCall" data-bs-placement="top" title="Video Call">
              <IconifyIcon icon='tabler:video' className="fs-20" />
            </Link>
            <Link to="" className="btn btn-sm btn-icon btn-ghost-light d-xl-flex">
      ...
      <div>
        <SimplebarReactClient className="chat-scroll p-3">
        ...
        <div className="p-3 border-top position-sticky bottom-0 w-100 mb-0">
          <form className="row align-items-center g-2" name="chat-form" id="chat-form" onSubmit={handleSubmit(sendChatMessage)}>
            <Col xs={'auto'}>
              <Dropdown drop="up">
                <DropdownToggle type="button" className="btn btn-icon btn-soft-warning content-none">
            ...
            <TextFormInput noValidate control={control} name="newMessage" containerClassName="col" className="form-control" placeholder="Enter your message" />
            <Col sm={'auto'}>
              <div className="d-flex align-items-center gap-1">
                <button type="submit" className="btn btn-icon btn-success"><IconifyIcon icon="tabler:send" /></button>
                <Link to="" className="btn btn-icon btn-soft-primary"><IconifyIcon icon="tabler:microphone" /> </Link>
                <Link to="" className="btn btn-icon btn-soft-primary"><IconifyIcon icon="tabler:paperclip" /></Link>
              </div>
            </Col>
          </form>
        </div>
