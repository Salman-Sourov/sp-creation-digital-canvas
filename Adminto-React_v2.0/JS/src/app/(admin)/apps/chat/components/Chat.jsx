...
const Chat = ({
  ...
  return <div className="chat-user-list border-end">
      <CardBody className="py-2 px-3 border-bottom">
        <div className="d-flex align-items-center gap-2 py-1">
          ...
          <div className="flex-grow-1">
            <h5 className="mb-0">
              <Link to="" className="text-reset lh-base">Dhanoo K.</Link>
            </h5>
          ...
          <button onClick={chatSetting.toggle} type="button" className="flex-grow-0 btn btn-sm btn-icon btn-soft-danger d-xl-none" data-bs-dismiss="offcanvas" data-bs-target="#chatUserList" aria-label="Close">
            <IconifyIcon icon='tabler:x' className="fs-20" />
          </button>
        </div>
      </CardBody>
      <div className="d-flex flex-column chat">
        <SimplebarReactClient className="users-list position-relative list-scroll">
          <div className="px-3 py-2">
            <div className="app-search py-1">
              <input type="text" className="form-control border-light bg-light bg-opacity-50 rounded-2" placeholder="Search something here..." />
          ...
          {users?.map((item, idx) => <Fragment key={idx}>
                <Link to="" className="text-body d-block">
                  <div className={`chat-users ${selectedUser?.id === item.id && 'active'}`} onClick={() => {
              onUserSelect(item);
            }}>
                    <div className="avatar-md chat-avatar-offline">
                      ...
                      {!item.image && (item.icon ? <div className={`h-100 w-100 rounded-circle bg-${item.iconColor} text-white d-flex align-items-center justify-content-center`}>
                              <IconifyIcon icon={item.icon} className="fs-20" />
                            </div> : <div className="h-100 w-100 rounded-circle bg-info text-white d-flex align-items-center justify-content-center">
                              <span className="fw-semibold">{getFirstTwoLetters(item.name)}</span>
                            </div>)}
                    </div>
                    <div className="flex-grow-1 overflow-hidden">
                      <h5 className="mt-0 mb-0 fs-13">
                        <span className="float-end text-muted fs-12">{item.date.toLocaleString('en-us', {
                      hour: '2-digit',
                      minute: '2-digit'
                    })}</span>
                        {item.name}
                      ...
                      <p className="mt-1 mb-0 text-muted lh-1">
                        {item.unRead && <span className="w-25 float-end text-end"><span className="badge bg-danger-subtle text-danger">{item.unRead}</span></span>}
                        {!item.unRead && (item.isSend ? <span className="w-25 text-end float-end text-success"><IconifyIcon icon="tabler-checks" /></span> : <span className="w-25 text-end float-end text-muted"><IconifyIcon icon="tabler-checks" /></span>)}

                        {selectedUser?.activeOffline === 'Typing,,,,' && selectedUser.id === item.id ? <span className="w-75 d-inline-block text-primary fs-12 fw-semibold">typing...</span> : <>
                ...
                {item.allMessage && <div className="d-flex align-items-center px-3 py-2 bg-body-secondary position-sticky top-0 z-1">
