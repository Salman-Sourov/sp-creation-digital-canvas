  ...
  return <div className="topbar-item">
      <Dropdown align={'end'}>
        ...
        minHeight: 300
            ...
            {notifications.map((item, idx) => <div className="notification-item dropdown-item py-2 text-wrap" id="notification-1" key={idx}>
                  <span className="d-flex align-items-center">
                    {item.image ? <span className="me-3 position-relative flex-shrink-0">
                          ...
                          {item?.icon && <span className={`avatar-title bg-${item?.icon.variant}-subtle text-${item?.icon.variant} rounded-circle fs-22`}>
          ...
          <Link to="" className="dropdown-item notification-item position-fixed z-2 bottom-0 text-center text-reset text-decoration-underline link-offset-2 fw-bold notify-item border-top border-light py-2">
            View All
          </Link>
        </DropdownMenu>
      </Dropdown>
